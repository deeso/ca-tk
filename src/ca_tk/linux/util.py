...
class NTDescToJson(object):
    ...
    def nt_fpregset(cls, note, idx=0):
        s = DataToStruct.nt_fpregset(note)
    ...
    def nt_x86_state(cls, note, idx=0):
        s = DataToStruct.nt_x86_state(note)
        r = json_serialize_struct(s) if s is not None else {}
    ...
    def nt_prstatus(cls, note, idx=0):
        s = DataToStruct.nt_prstatus(note)
        r = json_serialize_struct(s) if s is not None else {}
        for k in note:
            if k == 'n_desc':
                continue
    ...
    def nt_siginfo(cls, note, idx=0, is_amd64=True):
        s = DataToStruct.nt_siginfo(note, is_amd64)
        r = json_serialize_struct(s) if s is not None else {}
        for k in note:
            if k == 'n_desc':
                continue
    ...
    @classmethod
    def nt_auxv(cls, note, is_amd64=True):
        auxv = DataToStruct.nt_auxv(note, is_amd64)
        s_auxv = []
        r = {}
        idx = 0
        for _av in auxv:
